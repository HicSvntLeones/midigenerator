    ...
    def write_track(self, track_no):
        ...
        if track_no == 0:
            ...
            if self.metadata['instruments'] == 'default':
                self.event_list[track_no].append(('testthing', 0))#Fix this later
                self.event_list[track_no].append(('Control Change', 0, 0, 0, 0)) #changed to track 1 for testing
                self.event_list[track_no].append(('Program Change', 0, 0, 0)) #changed to track 1 for testing
            self.write_from_event_list(track_no)
            self.write_end_track(track_no)
