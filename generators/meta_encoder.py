...
class MetaEncoder:
    ...
    def write_track(self, track_no):
        ...
        if track_no == 0:
            ...
            self.event_list[track_no].append(('Set Tempo',self.metadata['bpm']))
            self.event_list[track_no].append(('Set Time Signature',self.metadata['time_sig_num'], self.metadata['time_sig_den'],24,8))
            if self.metadata['instruments'] == 'default':
                self.event_list[track_no].append(('Control Change', 0, 0))
                self.event_list[track_no].append(('Program Change', 0))
    ...
    def handle_note(self, event):
        ...
        note_off_event = ('Note Off', off_time, event['channel'], event['pitch'], 0)
